...
έγγραφα που ήρθαν από .doc — τα γνήσια .docx έχουν σωστή μορφοποίηση.
  ...
  2. Κενά <w:tcBorders></w:tcBorders> → αφαίρεση
     ...
     τίποτα, αλλά τα έγγραφα δεν έχουν στυλ πίνακα. Έσπαγε την προεπισκόπηση που
     έχει πια αφαιρεθεί· μένει γιατί ένα κενό στοιχείο χωρίς νόημα δεν έχει λόγο
     να ταξιδεύει μέσα στο παραδοτέο.
